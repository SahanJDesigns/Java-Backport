...
def extract_post_run_metrics(repo_dir: str, backport_commit: str, backport_parent: str, generated_patch: str = "") -> dict:
    ...
    The generated patch is what opencode returned in the YAML (or committed on top of backport_parent).
    """
    metrics = {
    ...
    try:
        ...
        real_patch = clean_patch(res.stdout.strip())
        metrics["old_version_real_patch"] = real_patch

        # --- Generated patch ---
        gen_patch = generated_patch
        if not gen_patch:
            res = subprocess.run(
        ...
        metrics["old_version_generated_patch"] = clean_patch(gen_patch).strip()

        metrics["is_100_percent_similar"] = (
            real_patch.strip() == metrics["old_version_generated_patch"]
        )
    except Exception as e:
...
def process_row(
    ...
    try:
        ...
        status, notes, patch_text = _parse_result_from_reply(llm_log, log_path)
        result["Status"] = status
        result["Notes"] = notes
        if patch_text:
            result["old_version_generated_patch"] = patch_text
    ...
    if result["Status"] not in ["error", "fatal_error"]:
        try:
            metrics = extract_post_run_metrics(repo_dir, backport_commit, backport_parent, result.get("old_version_generated_patch", ""))
...
def _parse_result_from_reply(text: str, log_path: Path):
    ...
    Returns (status, notes, patch) tuple.
    """
    if not text:
